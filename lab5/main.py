...
SEND_STOP_TIME = 1.0                # Czas po jakim wysyłana jest komenda stopu do Arduino. Jest to czas zużycia procesora.
SCALE_FACTOR = 4
KERNEL = (9, 9)
MINIMUM_POINTS_IN_CONTOUR = 15      # Minimalna ilość punktów w konturze potrzebnych do podjęcia próby wykrywania okręgów.
MINIMUM_MOMENT = 0.15               # Minimalna wyliczona wartość ze wzoru aby uznać kontur za okrąg.
INNER_RADIUS_MULTIPLIER = 0.9       # Promień wewnętrznego okręgu w toruse.
ROI_SIZE = (6, 6)

FIND_CONTOUR_METHOD = 0             # 0 - połączone, 1 - momenty, 2 - torus
...
def roi(image):
    return image[newHeight//2 - ROI_SIZE[0]//2 : newHeight //2 + ROI_SIZE[0]//2, newWidth//2 - ROI_SIZE[1]//2 : newWidth//2 + ROI_SIZE[1]//2, :]
...
def findCircle(mode, contour):
    if mode is 0:
        return checkIfCircleMomentsMethod(contour) and checkMinimumPointsInTorus(contour)
    elif mode is 1:
        return checkIfCircleMomentsMethod(contour)
    else :
        return checkMinimumPointsInTorus(contour)
...
while not quit():
    ...
    if contours:            
        for i, contour in enumerate(contours):
            if len(contour) >= MINIMUM_POINTS_IN_CONTOUR:
                if findCircle(FIND_CONTOUR_METHOD, contour):
    ...
    if filteredContours: 
        ...
        if DEBUG:
            ...
            center = (int(x),int(y))
            cv2.circle(upscaledColor, center, int(radius), BGR_ORANGE, thickness=1)
            radius = int(radius*INNER_RADIUS_MULTIPLIER)
            cv2.circle(upscaledColor, center, radius, BGR_RED, thickness=1)
    ...
    if biggestObject_BoundingBox:
        ...
        cv2.line(upscaledColor, screenMiddle, biggestObjectMiddle, BGR_RED)

        sendPacket('packet', scaled[0], scaled[1])
        stopMessage = time.process_time()
        if not ARDUINO_MODE:
            cv2.circle(upscaledColor, biggestObjectMiddle, 2, BGR_BLUE, thickness=2)
            cv2.rectangle(resizedColor, (x, y), (x+w, y+h), BGR_RED, thickness=2)
    else:
